...
def extract_vision_info_old(conversations: list[dict] | list[list[dict]]) -> list[dict]:
    vision_infos = []
            ...
            if isinstance(message["content"], list):
                for ele in message["content"]:
                    if (
                        ...
                        vision_infos.append(ele)
                    elif "points" in ele or ele.get("type", "") in ("points"):
                        points_infos.append(ele)
    return vision_infos, points_infos


def process_vision_info_old(
...
def fetch_point(ele):
    points = torch.from_numpy(np.load(ele["points"]))  # (8192,6)
    return points
...
def process_vision_info(
    ...
    vision_infos, points_infos = extract_vision_info(conversations)
    # print(points_infos)
    # [{'type': 'points', 'points': '/apdcephfs_jn/share_302245012/yjmaes/huggingface/8192_npy/ed51a51909ee46c780db3a85e821feb2_8192.npy'}]
    ## Read images or videos
    image_inputs = []
    video_inputs = []
    point_inputs = []
    ...
    for vision_info in vision_infos:
        if "image" in vision_info or "image_url" in vision_info:
            ...
            raise ValueError("image, image_url or video should in content.")
    for points_info in points_infos:
        if "points" in points_info:
            point_inputs.append(fetch_point(points_info))
    ...
    if return_video_kwargs:
        return image_inputs, video_inputs, {"fps": video_sample_fps_list}
    return image_inputs, video_inputs, point_inputs
